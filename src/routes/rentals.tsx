          ...
          <div className="rounded-3xl border border-border bg-card/80 p-6 shadow-card backdrop-blur-xl">
            ...
            {done ? (
              <div className="mt-6 rounded-2xl border border-emerald/40 bg-emerald/10 p-5 text-sm">
                <div className="flex items-center gap-2 font-semibold text-emerald"><CheckCircle2 className="h-5 w-5" /> Request received</div>
                <p className="mt-2 text-foreground">Reference <strong>{done.ref}</strong>. Our team will reach out via {channel === "whatsapp" ? "WhatsApp" : "email"} with your invoice.</p>
                <button onClick={()=>{setDone(null); setPick(""); setForm({full_name:"",company:"",email:"",phone:"",start_date:"",end_date:"",message:""});}} className="mt-4 text-xs font-semibold text-primary hover:underline">Submit another</button>
              </div>
            ) : (
              <form onSubmit={submit} className="mt-5 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className="flex items-center gap-1 rounded-lg border border-input bg-background px-2 text-xs"><Calendar className="h-3.5 w-3.5 text-muted-foreground" /><input type="date" value={form.start_date} onChange={(e)=>setForm({...form,start_date:e.target.value})} className="w-full bg-transparent py-2 text-sm outline-none" /></label>
                  <label className="flex items-center gap-1 rounded-lg border border-input bg-background px-2 text-xs"><Calendar className="h-3.5 w-3.5 text-muted-foreground" /><input type="date" value={form.end_date} onChange={(e)=>setForm({...form,end_date:e.target.value})} className="w-full bg-transparent py-2 text-sm outline-none" /></label>
                </div>
                <textarea placeholder="Project notes…" value={form.message} onChange={(e)=>setForm({...form,message:e.target.value})} rows={3} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />

                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={()=>setChannel("email")} className={`flex items-center justify-center gap-1 rounded-full px-3 py-2 text-xs font-semibold ${channel==="email"?"bg-gradient-hero text-primary-foreground":"border border-border"}`}><Mail className="h-3.5 w-3.5" /> Email me</button>
                  <button type="button" onClick={()=>setChannel("whatsapp")} className={`flex items-center justify-center gap-1 rounded-full px-3 py-2 text-xs font-semibold ${channel==="whatsapp"?"bg-emerald text-emerald-foreground":"border border-border"}`}><MessageCircle className="h-3.5 w-3.5" /> WhatsApp</button>
                </div>

                {err && <p className="rounded-md bg-destructive/10 px-3 py-2 text-xs text-destructive">{err}</p>}
                <button disabled={busy} className="w-full rounded-full bg-gradient-gold px-5 py-3 font-semibold text-gold-foreground shadow-soft hover:scale-[1.01] disabled:opacity-60">
...
function Input({p, v, on, type="text"}:{p:string; v:string; on:(s:string)=>void; type?:string}) {
  return <input placeholder={p} type={type} value={v} onChange={(e)=>on(e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />;
}